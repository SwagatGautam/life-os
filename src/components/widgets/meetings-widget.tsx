...
import { Video, Users, Clock, Loader2, Lock } from "lucide-react";
...
export function MeetingsWidget() {
  ...
  return (
    <div className="widget-card h-full flex flex-col relative overflow-hidden group">
      {!isPro && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center p-6 text-center backdrop-blur-[2px] bg-background/40 group-hover:backdrop-blur-[4px] transition-all">
          ...
          <h4 className="font-bold text-xs mb-1">PRO Feature</h4>
          <p className="text-[9px] text-muted-foreground mb-4 max-w-[140px]">Host video meetings and collaborate live with your team.</p>
          <Link href="/pricing" className="px-3 py-1 bg-green-500 text-white text-[9px] font-bold rounded-lg hover:opacity-90 transition-all">
            Unlock Now
          </Link>
        </div>
      )}

      <div className={cn("flex flex-col h-full transition-all duration-300", !isPro && "blur-[1px] opacity-40")}>
